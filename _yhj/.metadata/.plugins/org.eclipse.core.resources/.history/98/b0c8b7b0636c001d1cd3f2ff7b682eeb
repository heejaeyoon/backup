			...
			if (num == 0) {
		     cupon = (int)(Math.random() * 16 + 1);
				...
				if (money < total-(total/100*cupon)) {
					...
					System.out.println("잔돈은" + (money-total+(total/100*cupon)) + "원입니다.");
				} // 수중에 있는 금액 - 내야할 음식의 총 가격 + ( 쿠폰을 적용을 적용해서 받는 혜택금액)  
			
			
			} else if (num == 4) {
				new LoginApplication().start(args);
			    int totalPrice = total-(total/100*cupon);      //지불금액
		        int currentPoint = 0;               // 현재 포인트
		        int point = 0;            // 포인트 적립량
		        int savedPoint = 0;        // 적립된 포인트
		 
		        System.out.println("구매금액은" + totalPrice + "입니다.");
		        System.out.println("포인트 적립금:" + (totalPrice/100*5) + "원");
//		        System.out.println("현재 포인트를 입력하세요.");
//		        currentPoint = sc.nextInt();
		        
		        totalPrice = total-(total/100*cupon);
		        point = totalPrice/100*5;
		        savedPoint = currentPoint + point;

		        System.out.println("--------------------------");
		        System.out.println("총 구매가격 : " + totalPrice + "원");
		        System.out.println("--------------------------");
//		        System.out.println("적립전 포인트 : " + currentPoint + "포인트");
//		        System.out.println("적립 포인트 : " + point + "포인트");
//		        System.out.println("적립후 포인트 : " + savedPoint + "포인트");
		        System.out.println("--------------------------");
